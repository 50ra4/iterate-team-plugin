...
**dirty task worktree ガード（exit 3）**: merge スクリプトは取り込み前に `git -C <worktree-path> status --porcelain` で当該 task worktree のクリーン性を検証し、未コミット差分（generator の `git add` 漏れ・生成物 / rename 取りこぼし等）が残る場合は **マージせず** exit 3 + `DIRTY_TASK_WORKTREE: <task-id>` で終了する。これは「Evaluator が dirty worktree 内で未コミット差分込みの挙動を承認 → `git merge` はコミット済み内容しか統合しない → 後続 cleanup の `--force` が未コミットファイルを消失させる」という整合性崩壊を防ぐ構造ガードである。検出時は **エスカレーション**（自動解決・自動 `git add` 禁止。未コミット差分はタスク仕様外の混入か add 漏れか人間が判断）。runlog: `merge_dirty_worktree` / `{task_id, status_excerpt}`。**ステップ 5.6 のクリーンアップは保留**（未コミットファイルを人間調査用に温存）。既マージタスクは保持。ステップ 9。なお `team-worktree-setup.sh` が無条件で張る `node_modules` symlink（ルート + ネスト）は git pathspec `:(exclude)` で除外されるため、node_modules を gitignore しないプロジェクトでも誤検知しない。git status 自体が失敗した場合は clean とみなさず fail closed（exit 2）でエスカレーションする。
...
| `<knowledge_digest_path>`      | ステップ 0.0 で `<repo-root>/.iterate-team/knowledge/lessons.md` の絶対パスとして設定（ファイル不在でも保持）                                                                                                                                                                             | `team-planner` / `team-generator` / `team-evaluator` / `team-interviewer` / `team-test-coder` の起動プロンプトへの `knowledge_digest_path` 注入（`harness-common.md#knowledge-ダイジェスト注入全コマンド共通`）                          |
