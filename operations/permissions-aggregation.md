...
"allow": [
  ...
  // 2026-07-13 改訂 13 次: adapter Phase 2（フィードバック捕捉 + learned-rules ループ）追加 3 本
  "Bash(<plugin_root>/scripts/adapter-record-feedback.sh *)",
  "Bash(<plugin_root>/scripts/adapter-recover.sh *)",
  "Bash(<plugin_root>/vendor/agent-os/scripts/split-learned-rules.sh *)",
  // 2026-07-14 改訂 14 次: adapter Phase 3（自動観測 preflight、ステップ 0.3）追加 1 本。
  // read-only 検出器（.agent-os/ を一切書き換えない）
  "Bash(<plugin_root>/scripts/adapter-check-staleness.sh *)",
...
- **adapter Phase 2（フィードバック捕捉 + learned-rules ループ）関連 3 エントリの追加 (2026-07-13 改訂・13 次)**:
  - Phase 1（12 次）で `.agent-os/` の観測（`team-profiler`）と Write/Edit 権限を導入済み。本改訂は Phase 2（`team-adapter` による学習、`/iterate-review` ステップ 6.8。[`adapter-policy.md`](./adapter-policy.md) §6〜§8・[`iterate-team-runbook.md`](./iterate-team-runbook.md#ステップ-68-adapter-学習) 参照）の実行に必要なスクリプト呼び出し許可 3 本を group (2)（ハーネス補助スクリプト）へ追加する。
  - `Bash(<plugin_root>/scripts/adapter-record-feedback.sh *)`: `team-adapter` が `review-feedback-log.md` へユーザー訂正を逐語 1 件 append するための flock 安全なラッパー（`knowledge-append.sh` と同型の狙い）。stdin 経由で本文を受け取るため引数へ逐語テキストが露出せず、`--category` は 7 分類の固定 enum で検証される。
  - `Bash(<plugin_root>/scripts/adapter-recover.sh *)`: `.agent-os/` コミット失敗時の fail-open 復旧スクリプト（`knowledge-recover.sh` の構造クローン）。tracked 復元 + untracked 退避のみを行い、`.agent-os/` を書き換える性質の操作ではあるが、既存の `Write(.agent-os/**)`/`Edit(.agent-os/**)`（12 次）とは独立にコマンド実行そのものの許可が必要なため追加する。
  - `Bash(<plugin_root>/vendor/agent-os/scripts/split-learned-rules.sh *)`: fable からそのまま vendoring した read-only 寄りのスクリプト。`team-adapter` の `mode=adapt`（`/iterate-adapt` の reconcile パス）が `--dry-run` でのみ呼び、`learned-rules.md` の active ルールを `rules/<scope>.md` へ分割する移動プランを**提案として取得するのみ**で、本実行（分割の実体化）は行わない（`agents/team-adapter.md` の禁止事項「承認前の split-learned-rules.sh 本実行」）。
  - 捕捉（`.iterate-team/state/<session-id>/pending-feedback/` へのステージング）自体は既存の `Write(.iterate-team/state/*/**)`（group (4)）でカバーされるため、捕捉専用の新規権限は不要である。
  - 既存の push・secrets 防御（group (3) の git 系許可、`permissions.deny` の force/mirror/delete push 各形態・`git add .`/`-A`・`git reset --hard *`・`Read(~/.ssh/**)`/`Read(~/.aws/**)`）はいずれも本改訂の対象外であり、`deny` 27 件・`ask` 2 件は変更していない。

- **adapter Phase 3（自動観測 preflight）関連 1 エントリの追加 (2026-07-14 改訂・14 次)**:
  - Phase 3（`/iterate-plan`・`/iterate-team` ステップ 0.3。[`adapter-policy.md`](./adapter-policy.md)・[`iterate-team-runbook.md#03-adapter-preflight`](./iterate-team-runbook.md) 参照）は、preflight で `.agent-os/` の staleness（`fresh`/`absent`/`stale`）を判定するために `Bash(<plugin_root>/scripts/adapter-check-staleness.sh *)` を group (2) へ 1 本追加する。
  - `adapter-check-staleness.sh` は **read-only 検出器**であり、`<adapter_dir>`（`<project_dir>/.agent-os`）配下のファイルを `Read` するのみで一切の書込みを行わない（vendored read-only 検証スクリプト 2 本 `validate-agent-os.sh` / `detect-rule-conflicts.sh` と同じ性質のパターン、12 次で追加済み）。判定結果に応じた実際の書き込み（`--adapt` 指定時のインライン観測）は、既存の `adapter-bootstrap.sh`（インストーラ wrapper） / `Write(.agent-os/**)` / `Edit(.agent-os/**)`（12 次） / `adapter-recover.sh`（fail-open、13 次）を再利用するため、本改訂で新規に追加する書き込み系権限は無い。
  - 既存の push・secrets 防御（`permissions.deny` の force/mirror/delete push 各形態・`git add .`/`-A`・`git reset --hard *`・secrets 読取 deny、唯一の無プロンプト push 経路 `team-push-branch.sh`）はいずれも本改訂の対象外であり、`deny` 27 件・`ask` 2 件は変更していない。
...
### adapter Phase 2（フィードバック捕捉 + learned-rules ループ）権限の追加 (2026-07-13 改訂・13 次)

| 指標                                                                                     | Before (56 件) | After (59 件) | 変化 |
| ---------------------------------------------------------------------------------------- | -------------- | ------------- | ---- |
| allow エントリ数                                                                          | 56             | 59            | +3   |
| `Bash(<plugin_root>/scripts/adapter-record-feedback.sh *)`（新規、group (2)）             | 0              | 1             | +1   |
| `Bash(<plugin_root>/scripts/adapter-recover.sh *)`（新規、group (2)）                     | 0              | 1             | +1   |
| `Bash(<plugin_root>/vendor/agent-os/scripts/split-learned-rules.sh *)`（新規、group (2)） | 0              | 1             | +1   |
| deny エントリ数                                                                            | 27             | 27            | 0    |
| ask エントリ数                                                                             | 2              | 2             | 0    |

### adapter Phase 3（自動観測 preflight）権限の追加 (2026-07-14 改訂・14 次)

| 指標                                                                                     | Before (59 件) | After (60 件) | 変化 |
| ---------------------------------------------------------------------------------------- | -------------- | ------------- | ---- |
| allow エントリ数                                                                          | 59             | 60            | +1   |
| `Bash(<plugin_root>/scripts/adapter-check-staleness.sh *)`（新規、group (2)、read-only）  | 0              | 1             | +1   |
| deny エントリ数                                                                            | 27             | 27            | 0    |
| ask エントリ数                                                                             | 2              | 2             | 0    |
...
- **adapter Phase 2 追加のリスク評価（13 次）**: 追加した 3 エントリはいずれも `<plugin_root>` 配下の固定パスを指すスクリプト実行許可であり、対象リポジトリ側の任意パスを操作対象に取らない。`adapter-record-feedback.sh` は `--adapter <dir>` 引数で指定された `.agent-os/` 配下にのみ書き込み、本文は stdin 経由（引数へ逐語テキストが露出しない）、`--category` は固定 7 値の allowlist で検証される。`adapter-recover.sh` は fail-open 専用で、`git restore`/`git status`/`mv` による tracked 復元・untracked 退避のみを行い、対象パスは `.agent-os/` と `.iterate-team/state/<session-id>/failed-adapter/` に固定される（session-id はスクリプト内で `A-Za-z0-9._-` のみ・`..`/`/` 拒否のバリデーションを通る）。`split-learned-rules.sh` は `team-adapter` が `--dry-run` でのみ呼ぶため、allow に加えても `.agent-os/` への書き込みは発生しない（本実行は `agents/team-adapter.md` の禁止事項として構造的に閉じている）。3 本とも `.agent-os/` の書き込み主体を `team-profiler`/`team-adapter` の 2 agent に限定する 12 次の設計（`adapter-policy.md` §6）を変更しない。**既存の push・secrets 防御（`permissions.deny` の force/mirror/delete push 各形態・`git add .`/`-A`・`git reset --hard *`・secrets 読取 deny、および唯一の無プロンプト push 経路 `team-push-branch.sh`）はいずれも本改訂の対象外であり、`deny` 27 件・`ask` 2 件は変更していない**。
- **adapter Phase 3 追加のリスク評価（14 次）**: 追加した 1 エントリ（`adapter-check-staleness.sh`）は `<plugin_root>/scripts/` 配下の固定パスを指すスクリプト実行許可であり、`--adapter <dir>` で指定されたディレクトリ配下を `Read` するのみで一切の書込みを行わない（vendored `validate-agent-os.sh` / `detect-rule-conflicts.sh` と同型の read-only 検出器パターン）。`/iterate-plan`・`/iterate-team` ステップ 0.3 が既定（`--adapt` 未指定）で呼ぶ経路では、判定結果に応じた通知と `adapter_preflight` runlog マーカーの記録のみを行い `.agent-os/` への書き込みは発生しない。`--adapt` 指定時のインライン観測（`absent`/`stale` 検出時のみ）は 12 次で追加済みの `adapter-bootstrap.sh` / `Write(.agent-os/**)` / `Edit(.agent-os/**)` と 13 次で追加済みの `adapter-recover.sh`（fail-open）を再利用するのみで新規権限を要さず、書き込み主体は `team-profiler` に限定される（`adapter-policy.md` §6 の設計は不変）。**既存の push・secrets 防御はいずれも本改訂の対象外であり、`deny` 27 件・`ask` 2 件は変更していない**。
