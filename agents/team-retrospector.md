...
| `mode`         | `light` または `deep`                                                            |
| `session_list` | `mode=deep` のときのみ。横断対象の session-id 群                                  |
...
5. 記録する。1 件ずつ以下で append し、`id` は stdout から受領する（`ts`/`id` は script 側生成のため渡さない。更新レコードは既存 `id` を含めて append する）:
...
1. `session_list` の各セッションの `<state_root>/runlog.jsonl`（各セッションごとの絶対パスを `session_id` から導出）を `tail -n 400` で bounded read する（1 セッション分を丸ごと読み込まない）。
2. 横断で以下を行う:
...
5. `<knowledge_dir>/proposals/INDEX.md` を更新する（提案一覧）。
